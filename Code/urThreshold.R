...
save(mod.dat, file=paste0(country,'_cluster_dat_1frame.rda'), envir = .GlobalEnv)
...
constr_prior <- function(obs,jitter_r,prob_r,poly_admin,pop_ras){
  ...
  tryCatch({
    ...
    sp_xy <- sp::SpatialPoints(as.data.frame(obs)[,c('LONGNUM','LATNUM')],
                               proj4string = CRS(sp::proj4string(poly_admin)))
...
get_subnatl_frac<-function(adm.names, adm.idx, wp, poly_file, wp_adm=NULL, urb_vec){
  ...
  poly_file <- sp::spTransform(poly_file, wp@crs)
...
if('DHS' %in% mod.dat$survey.type){  
  ...
  for( i in 1:dim(urban_clus)[1]){
    ...
    p_mode = sqldf::sqldf('SELECT * FROM temp_frame GROUP BY x,y ORDER BY SUM(unn_w) DESC LIMIT 1')
...
for (year in pop.year){
  ...
  pop_f_0 <- raster::raster(f_0_name)
  pop_f_1 <- raster::raster(f_1_name)
  pop_m_0 <- raster::raster(m_0_name)
  pop_m_1 <- raster::raster(m_1_name)
  
  
  sp::proj4string(pop_f_0) <- sp::proj4string(pop_f_1) <- 
    sp::proj4string(pop_m_0) <- sp::proj4string(pop_m_1) <- 
    sp::proj4string(poly.adm1)
  ...
  raster::writeRaster(pop_u1, overwrite=TRUE, paste0(pop.abbrev,'_u1_',year,'_100m','.tif'))
  
  pop_surf <- raster::raster(paste0(country.abbrev,'_u1_',year,'_100m.tif'))
  
  
  pop_u1_aggregate <- aggregate(pop_surf, fact=10,sum)
  
  pop_grid$u1_pop <- raster::extract(pop_u1_aggregate, pop_grid[c('x','y')])
  
  u1_pop<-worldpop
  values(u1_pop)<-pop_grid$u1_pop 
  
  raster::writeRaster(u1_pop, overwrite=TRUE, paste0(country.abbrev,'_u1_',year,'_1km.tif'))
  
  #U5 Population
  pop_u5<- pop_f_0+pop_f_1+pop_m_0+pop_m_1
  
  raster::writeRaster(pop_u5, overwrite=TRUE,
              paste0(pop.abbrev,'_u5_',year,'_100m','.tif'))
  
  pop_surf <- raster::raster(paste0(country.abbrev,'_u5_',year,'_100m.tif'))
  
  
  pop_u5_aggregate <- aggregate(pop_surf, fact=10,sum)
  
  pop_grid$u5_pop <- raster::extract(pop_u5_aggregate, pop_grid[c('x','y')])
  
  u5_pop<-worldpop
  values(u5_pop)<-pop_grid$u5_pop 
  
  raster::writeRaster(u5_pop, overwrite=TRUE, paste0(country.abbrev,'_u5_',year,'_1km.tif'))
...
points.frame <- sp::SpatialPoints(points.frame)
...
write.xlsx(ref.tab, file='prepared_dat/reference_table.xlsx', rowNames = FALSE)
...
for ( t in 1:length(years)){
  ...
  u1_pop<-raster::raster(paste0(country.abbrev,'_u1_',year,'_1km.tif'))
  
  # national urban fraction for U1 population at year t
  u1_natl <- sum(urb_grid$urb_ind*values(u1_pop),na.rm=TRUE)/sum(values(u1_pop),na.rm=TRUE)
  ...
  u5_natl <- sum(urb_grid$urb_ind*values(u5_pop),na.rm=TRUE)/sum(values(u5_pop),na.rm=TRUE)
