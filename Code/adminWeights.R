...
pop_adm<-function(adm.shp, wp,admin_pop_dat){
  ...
  adm.shp <- sp::spTransform(adm.shp, wp@crs)
...
rigorousFileTest = FALSE # set to TRUE after files have been downloaded to test 
...
for (year in pop.year){
  ...
  worldpop <- raster::raster(paste0(country.abbrev, '_ppp_',year, '_1km_Aggregated_UNadj.tif',sep=''))
...
for(year in pop.year){
  ...
  pop_u5<- raster::raster(paste0(data.dir,'/Population/',country.abbrev,'_u5_',year,'_100m','.tif'))
  pop_u1<- raster::raster(paste0(data.dir,'/Population/',country.abbrev,'_u1_',year,'_100m','.tif'))
